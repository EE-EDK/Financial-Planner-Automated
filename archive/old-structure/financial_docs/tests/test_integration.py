...
class TestHealthScore:
    ...
    def test_score_components(self):
        ...
        ef_score = chs.calculate_emergency_fund_score()
        debt_score = chs.calculate_debt_management_score()
        sr_score = chs.calculate_savings_rate_score()
        cf_score = chs.calculate_cash_flow_score()

        # Each should return dict with score and max_score
        for score in [ef_score, debt_score, sr_score, cf_score]:
    ...
    def test_total_score_range(self):
        ...
        ef = chs.calculate_emergency_fund_score()
        debt = chs.calculate_debt_management_score()
        sr = chs.calculate_savings_rate_score()
        cf = chs.calculate_cash_flow_score()

        total = ef['score'] + debt['score'] + sr['score'] + cf['score']
        assert 0 <= total <= 100
    ...
    def test_grade_assignment(self):
        ...
        assert chs.get_grade(95) == 'A'
        assert chs.get_grade(85) == 'B'
        assert chs.get_grade(75) == 'C'
        assert chs.get_grade(65) == 'D'
        assert chs.get_grade(55) == 'F'


class TestScenarioCalculator:
    """Integration tests for scenario_calculator.py."""

    def test_sell_property_scenario(self):
        """Sell property scenario should calculate correctly."""
        import scenario_calculator as sc

        result = sc.calculate_scenario_sell_property()
    ...
    def test_furlough_scenario(self):
        """Furlough scenario should calculate correctly."""
        import scenario_calculator as sc

        result = sc.calculate_scenario_furlough()

        assert 'income_loss' in result
        assert 'deficit' in result
        assert 'months_covered' in result
        assert 'mitigation' in result

    def test_perfect_execution_scenario(self):
        """Perfect execution should have projections."""
        import scenario_calculator as sc

        result = sc.calculate_scenario_perfect_execution()

        assert 'projections' in result
        assert 6 in result['projections']
        assert 12 in result['projections']
        assert 36 in result['projections']
